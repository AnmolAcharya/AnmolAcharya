<h1 align="center">Hi 👋, I'm Anmol Acharya</h1>
<h3 align="center">Focused on ☁️ Cloud & DevOps | 💻 Software Engineering | 🤖 AI/ML</h3>
<h3 align="center">My LinkedIn Profile: https://www.linkedin.com/in/anmolacharyauta </h3>
<h3 align="center">Connect with me: anmolacharya94@gmail.com</h3>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=your-username&label=Profile%20views&color=0e75b6&style=flat" alt="profile views" />
</p>

---

## ☁️ Cloud & DevOps

- 🚀 Building scalable, serverless, and containerized applications on AWS
- 🛠️ Experienced with AWS Lambda, API Gateway, DynamoDB, S3, CloudFront, and Cognito
- 🔄 Automated CI/CD pipelines using GitHub Actions & CodePipeline
- 🧩 Passionate about infrastructure as code (Terraform, CloudFormation)

---

## 💻 Software Engineering

- 🧑‍💻 Full-stack developer with React (frontend) and Java/FastAPI (backend)
- ⚡ Built real-time multiplayer games and SaaS tools
- 🧹 Focus on clean code, scalable architecture, and robust testing (JUnit, Pytest)
- 🌟 Love working on meaningful products that impact users

---

## 🤖 AI / ML

- 🔬 Researcher in AI-powered parasite detection systems (YOLOv8, blood smear images)
- 📈 Experience with explainable AI techniques (Grad-CAM, LayerCAM)
- 🤝 Merging ML models into production using FastAPI and AWS
- 💡 Interested in generative AI, NLP, and advanced computer vision

---

## 💬 Connect with me

<p align="left">
  <a href="https://linkedin.com/in/your-linkedin" target="blank"><img align="center" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/linkedin/linkedin-original.svg" alt="linkedin" height="30" width="40" /></a>
  <a href="mailto:your-email@example.com" target="blank"><img align="center" src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/google/google-original.svg" alt="email" height="30" width="40" /></a>
</p>

---

## 🛠️ Languages & Tools

<p align="left">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" alt="python" width="40" height="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg" alt="javascript" width="40" height="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/react/react-original.svg" alt="react" width="40" height="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/aws/aws-original.svg" alt="aws" width="40" height="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/docker/docker-original.svg" alt="docker" width="40" height="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" alt="github" width="40" height="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg" alt="mysql" width="40" height="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/java/java-original.svg" alt="java" width="40" height="40"/>
</p>

---


<p align="center">Thanks for stopping by! ⭐️</p>
